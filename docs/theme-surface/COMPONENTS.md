...
| C13 | 详情面板删除钮（lib） | `.btn.btn-sm.danger`（常亮红字红边） | ~~追加 `ghost` chrome~~ **已批准偏离（Task 10）**：不追加 `ghost`、不改 JS 类名——`library-vocab.js:513`/`library-notes.js:405`/`library.html:122` 三处 `class="btn btn-sm danger"` 原样保留，`.btn.danger` 就地重定义为 quiet 档（仅换 `color`/`border-color` token，chrome 不变） | 前景色换成 `--lib-danger-quiet-fg`（个别主题下会变，见 C12）；**geometry/chrome 不变**——按钮仍带底、带边，不是原设想的透明 ghost。是否已经"够安静"是人审判断题（附录 A2），本表如实记录已交付的是哪一半 | 本战役 |
