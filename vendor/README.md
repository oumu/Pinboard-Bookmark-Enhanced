...
|------|---------|----------|---------|---------|
| `defuddle.js` | 0.19.2 | https://github.com/kepano/defuddle | MIT | Extracts main article content from arbitrary web pages so AI tag/summary requests can send clean text instead of full HTML. Injected via `chrome.scripting.executeScript` into the active tab on user action only. |
...
| `marked.min.js` | 18.0.7 | https://github.com/markedjs/marked | MIT | Converts canonical Markdown to HTML for the preview render (then sanitized by DOMPurify). Loaded only inside `md-preview.html`. Vendored from the npm UMD build (`lib/marked.umd.js`) under the historical `.min.js` name. |
...
### `defuddle.js`
```bash
npm pack defuddle@0.19.2
tar -xzf defuddle-0.19.2.tgz package/dist/index.js
...
### `marked.min.js`
```bash
npm pack marked@18.0.7
tar -xzf marked-18.0.7.tgz package/lib/marked.umd.js
